...
(module typed-tests typed/racket/no-check
  ...
  (require/typed
   racket/date
   [find-seconds (-> Integer Integer Integer Integer Integer Integer Boolean Integer)])
  ...
  (check-true (vector-andmap (λ ([x : Integer] [y : Integer]) (and (= x 1) (= y 2)))
                             #(1 1 1 1)
                             #(2 2 2 2)))
  (check-false (vector-andmap (λ ([x : Integer] [y : Integer]) (and (= x 1) (= y 2)))
                              #(1 1 1 1)
                              #(2 1 2 2)))
  ...
  (check-true (vector-ormap (λ ([x : Integer] [y : Integer]) (and (= x 1) (= y 2)))
                            #(0 0 1 0)
                            #(0 0 2 0)))
  (check-false (vector-ormap (λ ([x : Integer] [y : Integer]) (and (= x 1) (= y 2)))
                             #(0 0 1 0)
                             #(0 2 0 0)))
  
  )

(module untyped-tests racket
  (require rackunit
           plot/no-gui
           plot/private/gui/worker-thread)
