...
(define (do-plot-inverted output-fn)
  (output-fn (list (function sqr 1 7)
                   (error-bars (list (vector 4 2 12)
                                     ...
                                     (vector 36 6 10))
                               #:invert? #t))
             #:title "Error Bars Inverted"))

(define-runtime-path pr59-not-inverted-data "./test-data/pr59-1.dat")
(define-runtime-path pr59-inverted-data "./test-data/pr59-2.dat")
